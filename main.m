...
% lrImitation > lrCritic > lrActor % and usually by roughly one order of magnitude or more

mazelist = [1 2 3 4 ]; %I increase training for maze 4 and 5 to get better results



pretrainepisodes = 5000;             % imitation only, on all mazes

hybridepisodes   = episodes - pretrainepisodes;  % rest is hybrid, no pure RL

controller_output_percentage_end = 0;
test_controller_percent = controller_output_percentage_end;  % final test is actor only
...
for ep = 1:episodes
    ...
    %% Here we print after every 100 episodes, could lower it but computationally very expensive
...
for maze_id = 1:length(mazes)+1
    ...
    for idx_start_goal= 1:2
        ...
        if iter_test == 3 || iter_test == 4 || iter_test == 5 || iter_test == 6
            ylim([-30,13]);
        end
